...
public class ActorShapeLine extends Actor {
    ...
    public void draw(Batch batch, float parentAlpha) {
        ...
        // Выводит оставшиеся фигуры
        for (int i = 0; i < Math.min(9, score.getTask().size()); i++) {
            if (!score.getTask().elementAt(i).isGrabbed()) {
                // ROTATION
                // batch.draw(shapes[score.getTask()[i].getNumber_shape()], 30 + getX() + i * 50, getY() + 60, 24, 24, 48, 48, 1, 1, rotation_array_to_elements[i]);

                // NO ROTATION
                batch.draw(shapes[score.getTask().elementAt(i).getNumber_shape()], 30 + getX() + i * 50, getY() + 60, 24, 24, 48, 48, 1, 1, 0);
            } else {
                batch.setColor(1, 1, 1, 0.2f);
                batch.draw(shapes[score.getTask().elementAt(i).getNumber_shape()], 30 + getX() + i * 50, getY() + 60, 18, 18, 36, 36, 1, 1, 0);
            }
            batch.setColor(1, 1, 1, 1);
        }
